...
兜兜转转终于又回到了leetcode来。已经来到这武大读研了，心态爆炸的时候就回来看看leetcode。<br>
听说找工作之类的，C++比python好找多了，所以还是我想最好刷点C++，把这个经验刷上去。<br>
先把[题目0001](https://leetcode-cn.com/problems/two-sum)和[题目0002](https://leetcode-cn.com/problems/add-two-numbers)的C++版做出来然后提交了，github的目录结构也作了更改。<br>
后续估计要么多做几题，要么把已经做好的题写成C++版本。

### 2020.12.04 傍晚

那边大数据平台上忙着跑代码，闲得无聊过来刷个题哈。<br>
[题目0016](https://leetcode-cn.com/problems/3sum-closest/)很难，本人没文化，直接暴力过，以后有文化了或许会回来康康。<br>
python暴力法过不了，C++能过。讲真，这不公平，我寻思是不是应该将python的超时判定设得宽松一点？<br>
[题目0017](https://leetcode-cn.com/problems/letter-combinations-of-a-phone-number/)看起来有点唬人，后来想想暴力法应该就是列表的一个不断更新的过程，只不过这个更新与平常的增长、剪短之类的有所区别，所以先定义好一次的更新，以后调用之即可。<br>
然后，就这还超越了98%的用户，实在略无语。<br>
再用C++实现一个康康还有没有98%。<br>
这次更离谱，直接100%。芜湖\~

### 2020.12.05 晚上

[题目0019](https://leetcode-cn.com/problems/remove-nth-node-from-end-of-list)甚简单，不知为何是*中等*题。<br>
初步之想法，一次遍历求出链表的长度，第二遍把那个要删除的节点给处理了，试试提交之没想到就已经击败了93%的用户，这让我并没有什么动力去搞那个什么*进阶*挑战了。<br>
接下来就试试用这个*随机一题*的功能，抽中个[题目0268](https://leetcode-cn.com/problems/missing-number/)，算*简单*题，试秒之，然后下班。<br>
不考虑复杂度自然简单，如果考虑一下的话难度回稍微高一点点。暴力方法想必不用介绍了，而所谓*线性复杂度*的方法，我能想到的就是先准备一个[0,n]的表，遇到一个就勾一个，最后剩下的就是缺的了。<br>
我这原理自然是对的，但是本人C++水平有限，并没有在C++中提交成功，反而在python中以4行代码解决了，并击败了76%的用户。看来回去得学学C++的一些细节东西才行。<br>
后来回宿舍吼闲得无聊，又用python实现了[题目0019](https://leetcode-cn.com/problems/remove-nth-node-from-end-of-list)的原始任务以及*进阶*任务（进阶的代码击败了99.5%，甚爽）。

### 2020.12.06 凌晨

[题目0020]([20. 有效的括号 - 力扣（LeetCode） (leetcode-cn.com)](https://leetcode-cn.com/problems/valid-parentheses/))被标作*简单*，那么我们来看看有多简单。对于熟悉数据结构的同志来说，这个应该就是用一个栈结构就能够完美解决的——遇到左括号时入栈，遇到对应的右括号时出栈，最后栈如果空了就说明字符串是合法的。<br>现在难就难在什么地方呢，就是说用python撸出个栈的结构并不容易；而C++的栈的用法我也不太熟悉。<br>可能先用python试试手吧？<br>实际上操作起来的话，python的list类型当成栈来使用就挺好使的了，毕竟它是支持从末端入和出的。<br>在许多次犯些粗心大意的错误之后，一下子干趴下了96%的用户。还不错，前20题就除了18题比较难之外，其他的都已经刷好了。